import {getScores} from '../ipcHandlers/HeatRaceHandler';
...
export function calculateBoatScores(
  results: Result[],
  event_id: any,
...
sortedScores.sort((a, b) => {
  const initialComparison = a.scores.reduce((acc: number, score: number, index: number) => {
    if (acc !== 0) return acc;
